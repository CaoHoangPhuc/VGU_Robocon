...
import smbus,time,sys,numpy as np
...
class MPU9250:
    ...
    def __init__(self, address=SLAVE_ADDRESS):
        ...
        self.timestamp = 0
        self.GyroOff   = [0.157, -0.681, -0.257] 
        self.MagMaxMin = [[21.591, 41.833, 20.242],
                         [-42.883, -19.342, -40.633]]
        ...
        self.H = [0,0,0,0,0,0]#y,p,r,ycoe,dy,fy
        print("MPU Ok! Heading = ",self.fusion())
    ...
    def configAK8963(self, mode, mfs):
        ...
        self.magXcoef = 1#(data[0] - 128) / 256.0 + 1.0
        self.magYcoef = 1#(data[1] - 128) / 256.0 + 1.0
        self.magZcoef = 1#(data[2] - 128) / 256.0 + 1.0
    ...
    def readMagnet(self):
        ...
        x = self.dataConv(data[0], data[1])*self.magXcoef
        y = self.dataConv(data[2], data[3])*self.magYcoef
        z = self.dataConv(data[4], data[5])*self.magZcoef

        x = round(x * self.mres, 3) - self.MagBias[0]
        y = round(y * self.mres, 3) - self.MagBias[1]
        z = round(z * self.mres, 3) - self.MagBias[2]
    ...
    def read(self,c = 0):
        data = bus.read_i2c_block_data(self.address, ACCEL_OUT, 6) if c<1 else 0

        ax = self.dataConv(data[1], data[0])*self.ares if c<1 else 0
        ay = self.dataConv(data[3], data[2])*self.ares if c<1 else 0
        az = self.dataConv(data[5], data[4])*self.ares if c<1 else 0

        data = bus.read_i2c_block_data(self.address, GYRO_OUT, 6) if c<2 else 0

        gx = self.dataConv(data[1], data[0])*self.gres-self.GyroOff[0] if c<2 else 0
        gy = self.dataConv(data[3], data[2])*self.gres-self.GyroOff[1] if c<2 else 0
        gz = self.dataConv(data[5], data[4])*self.gres-self.GyroOff[2] if c<2 else 0

        data = bus.read_i2c_block_data(AK8963_SLAVE_ADDRESS, AK8963_MAGNET_OUT, 7) if c<3 else 0

        mx = self.dataConv(data[0], data[1])*self.magXcoef*self.mres-\
        	 self.MagBias[0] if c<3 else 0
        my = self.dataConv(data[2], data[3])*self.magYcoef*self.mres-\
        	 self.MagBias[1] if c<3 else 0
        mz = self.dataConv(data[4], data[5])*self.magZcoef*self.mres-\
        	 self.MagBias[2] if c<3 else 0
    ...
    def calibrate_Gyro(self):
    	print("Calib Gyro...")    	
    	self.GyroOff = [0,0,0]; GyroSet = [0,0,0]; ex = 3600
    	for i in range(0,ex):
    		data = self.readGyro()
    		GyroSet[0]+= data['x']
    		GyroSet[1]+= data['y']
